...
## declare variables
MYSQL_CONTAINER_NAME="{{cookiecutter.mysql_container_name}}"
MYSQL_DBNAME="{{cookiecutter.mysql_db_name}}"
MYSQL_USER="{{cookiecutter.mysql_user_name}}"
...
docker run --name="$MYSQL_CONTAINER_NAME" --env="MYSQL_ROOT_PASSWORD=root" -p {{cookiecutter.mysql_port}}:3306 --health-cmd='mysqladmin ping --silent' -d mysql
